...
exports.getSettlementData = void 0;
const Trip_1 = __importDefault(require("../models/Trip"));
const Expense_1 = __importDefault(require("../models/Expense"));
const getSettlementData = async (tripId) => {
    // Check trip
    const trip = await Trip_1.default.findById(tripId).populate("members", "fullName email");
    if (!trip) {
        throw new Error("Trip not found");
    }
    // Get all expenses
    const expenses = await Expense_1.default.find({ trip: tripId });
    // Total Expense
    ...
    expenses.forEach((expense) => {
        ...
        paidMap.set(userId, (paidMap.get(userId) || 0) + expense.amount);
    });
    // Balance Sheet
...
exports.getSettlementData = getSettlementData;
